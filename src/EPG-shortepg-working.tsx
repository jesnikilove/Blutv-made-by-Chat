  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const epgChannels = streamData.filter((c:any)=>c.epg_channel_id).sort((a:any,b:any)=>a.name.localeCompare(b.name));
        

        const guideMap: Record<string, any[]> = {};
        await Promise.all(
          streamData.map(async (channel: any) => {
            try {
              ...
              localStorage.setItem("EPG_DEBUG", JSON.stringify(epg,null,2));
        ...
        const sortedChannels = [...epgChannels].sort((a:any,b:any)=>{ const aGuide=(guideMap[a.stream_id]||[]).length>0?0:1; const bGuide=(guideMap[b.stream_id]||[]).length>0?0:1; return aGuide-bGuide; });
